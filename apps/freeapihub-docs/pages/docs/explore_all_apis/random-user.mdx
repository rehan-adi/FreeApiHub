...
```shell
https://freeapihub.onrender.com/api/v1/randomusers/:id
```
...
const id = "1a2b3c4d"; // Replace with actual user ID

axios
  .get(`https://freeapihub.onrender.com/api/v1/randomusers/${id}`)
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
