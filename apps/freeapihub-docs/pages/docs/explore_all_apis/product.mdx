...
To retrieve product data by ID, you can use the following endpoint:

```shell
https://freeapihub.onrender.com/api/v1/product/:id
```

Replace :id with the actual product ID you want to fetch.
...
axios
  .get(`https://freeapihub.onrender.com/api/v1/product/${productId}`)
  .then((response) => {
    console.log(response.data);
  })
  .catch((error) => {
